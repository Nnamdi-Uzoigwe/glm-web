        ...
        <div className="max-w-5xl mx-auto px-6 py-10">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <h1 className="font-serif text-white text-2xl">{session.user.name ?? session.user.email}</h1>
                <p className="text-white/50 text-sm">{session.user.email}</p>
              </div>
            </div>
            ...
            <form action={async () => {
              ...
              <button type="submit" className="flex items-center gap-2 text-white/60 hover:text-white text-sm transition-colors border border-white/20 hover:border-white/40 px-4 py-2 rounded-xl">
                <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M17 16l4-4m0 0l-4-4m4 4H7m6 4v1a3 3 0 01-3 3H6a3 3 0 01-3-3V7a3 3 0 013-3h4a3 3 0 013 3v1" />
                </svg>
                Sign Out
          ...
          <div className="grid grid-cols-3 gap-4 mt-8">
            {[
              ...
              {
                label: "Lessons Completed",
                value: userPurchases.reduce((acc, p) => acc + (p.completedLessons?.length ?? 0), 0),
              },
              {
                label: "Member Since",
              ...
              <div key={stat.label} className="bg-white/10 border border-white/15 rounded-xl px-5 py-4 text-center">
                <p className="text-2xl font-serif text-white font-bold">{stat.value}</p>
                <p className="text-white/50 text-xs mt-1">{stat.label}</p>
              </div>
            ))}
      ...
      <div className="max-w-5xl mx-auto px-6 py-10">
        ...
        {userPurchases.length === 0 ? (
          <div className="bg-white rounded-2xl border border-gray-100 p-12 text-center">
            ...
            <h3 className="font-serif text-[#1B3A6B] text-lg mb-2">No courses yet</h3>
            <p className="text-gray-400 text-sm mb-6">Enrol in a course to start your learning journey.</p>
            <a href="/courses" className="inline-block bg-[#1B3A6B] hover:bg-[#C9902A] text-white text-sm font-semibold px-6 py-3 rounded-xl transition-all duration-300">
              Browse Courses →
            </a>
          </div>
        ) : (
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-5">
            {userPurchases.map((purchase) => {
              const completed = purchase.completedLessons?.length ?? 0;
              return (
                <div key={purchase.id} className="bg-white rounded-2xl border border-gray-100 p-5 hover:shadow-md transition-all duration-300">
                  <div className="w-10 h-10 bg-[#1B3A6B]/8 rounded-xl flex items-center justify-center mb-4">
                    <svg className="w-5 h-5 text-[#1B3A6B]" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.8} d="M12 6.253v13m0-13C10.832 5.477 9.246 5 7.5 5S4.168 5.477 3 6.253v13C4.168 18.477 5.754 18 7.5 18s3.332.477 4.5 1.253m0-13C13.168 5.477 14.754 5 16.5 5c1.747 0 3.332.477 4.5 1.253v13C19.832 18.477 18.247 18 16.5 18c-1.746 0-3.332.477-4.5 1.253" />
                    </svg>
                  </div>
                  <h3 className="font-semibold text-[#1B3A6B] text-sm mb-1 line-clamp-2">{purchase.courseTitle}</h3>
                  <p className="text-xs text-gray-400 mb-4">{completed} lesson{completed !== 1 ? "s" : ""} completed</p>
                  <div className="w-full bg-gray-100 rounded-full h-1.5 mb-4">
                    <div className="h-1.5 bg-[#C9902A] rounded-full" style={{ width: completed > 0 ? "40%" : "0%" }} />
                  </div>
                  <a
        ...
        <div className="grid sm:grid-cols-3 gap-4">
          {[
            ...
            <a key={link.label} href={link.href} className="bg-white border border-gray-100 hover:border-[#1B3A6B]/20 hover:shadow-sm rounded-2xl px-5 py-4 flex items-center gap-3 transition-all duration-200">
              <span className="text-2xl">{link.icon}</span>
              <span className="text-sm font-semibold text-[#1B3A6B]">{link.label}</span>
            </a>
          ))}
