...
export default function EventsHero({ nextEvent }: { nextEvent?: NextEvent }) {
  const countdown = useCountdown(nextEvent?.date ?? "");
  ...
  return (
    <section className="relative bg-[#1B3A6B] overflow-hidden">
      ...
      <div className="relative max-w-6xl mx-auto px-6 py-20 lg:py-28">
        <div className="grid lg:grid-cols-2 gap-12 items-center">
          <div>
            <span className="inline-block text-[#C9902A] text-xs font-semibold tracking-[3px] uppercase mb-4">What's On</span>
            <h1 className="text-4xl lg:text-5xl font-serif text-white leading-tight mb-4">Events &<br />Gatherings</h1>
            <div className="w-14 h-1 bg-[#C9902A] rounded-full mb-5" />
            <p className="text-white/70 text-lg leading-relaxed">
              From city-wide outreaches to intimate training days — there's always something happening at Gospel Light. Come and be part of it.
            </p>
          </div>

          {nextEvent ? (
            <div className="bg-white/10 border border-white/20 rounded-2xl p-6 lg:p-8">
              <p className="text-[#C9902A] text-xs font-semibold uppercase tracking-widest mb-4">Next Event</p>
              <h3 className="font-serif text-white text-xl lg:text-2xl leading-snug mb-2">{nextEvent.title}</h3>
              <div className="flex flex-wrap items-center gap-3 text-xs text-white/60 mb-6">
                <span className="flex items-center gap-1.5">
                  <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z" />
                ...
                <span>{nextEvent.time}</span>
                <span>•</span>
                <span>{nextEvent.isOnline ? "🌐 Online" : `📍 ${nextEvent.location}`}</span>
              </div>
              <div className="flex gap-3 mb-6">
                <CountdownUnit value={countdown.days} label="Days" />
                <CountdownUnit value={countdown.hours} label="Hours" />
              ...
              <div className="flex items-center gap-2">
                <span className={`text-xs font-semibold px-2.5 py-1 rounded-full ${nextEvent.isFree ? "bg-green-500/20 text-green-300" : "bg-[#C9902A]/20 text-[#C9902A]"}`}>
                  {nextEvent.isFree ? "Free Entry" : "Ticketed"}
                </span>
                <span className="text-xs text-white/50">
                  {nextEvent.capacity - nextEvent.registered} spots remaining
                </span>
            ...
            <div className="bg-white/10 border border-white/20 rounded-2xl p-6 lg:p-8 text-center">
              <p className="text-white/50 text-sm">No upcoming events at the moment.</p>
