...
export default async function AdminLayout({
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      <div className="bg-[#1B3A6B] border-b border-white/10 top-0 z-50 relative">
        <div className="max-w-7xl mx-auto px-4 sm:px-6">
          {/* Top row */}
          <div className="flex items-center justify-between h-14">
            <div className="flex items-center gap-3">
              ...
              <p className="text-white font-semibold text-sm sm:hidden">GLM Admin</p>
            </div>
            ...
            <div className="flex items-center gap-2">
              ...
              <a href="/" className="hidden sm:flex items-center gap-1.5 text-white/50 hover:text-white/80 text-xs px-3 py-1.5 rounded-lg transition-all hover:bg-white/10">
                <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M10 19l-7-7m0 0l7-7m-7 7h18" />
                </svg>
                Back to site
              ...
              <form action={async () => {
                ...
                <button type="submit" className="flex items-center gap-1.5 text-white/60 hover:text-white text-xs px-3 py-1.5 rounded-lg border border-white/20 hover:border-white/40 transition-all hover:bg-white/10">
                  <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M17 16l4-4m0 0l-4-4m4 4H7m6 4v1a3 3 0 01-3 3H6a3 3 0 01-3-3V7a3 3 0 013-3h4a3 3 0 013 3v1" />
                  </svg>
                  <span className="hidden sm:inline">Sign Out</span>
          ...
          {/* Desktop nav row
          <div className="hidden sm:flex items-center gap-1 overflow-x-auto pb-2 scrollbar-hide">
            {navItems.map((item) => (
              <a key={item.href} href={item.href} className="text-white/60 hover:text-white hover:bg-white/10 px-3 py-1.5 rounded-lg text-xs sm:text-sm whitespace-nowrap transition-all shrink-0">
                {item.label}
              </a>
            ))}
          </div> */}
